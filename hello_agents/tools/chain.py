...
class ToolChain:
    ...
    def add_step(self, tool_name: str, input_template: str, output_key=None):
        """
        添加工具执行步骤
...
class ToolChainManager:
    ...
    def register_chian(self, chain: ToolChain):
        """注册工具链"""
...
def create_research_key():
    ...
    chain.add_step(
        tool_name="search", input_template="{input}", output_key="search_key"
    )

    chain.add_step(
        tool_name="my_calculator", input_template="2 + 2", output_key="calc_result"
    )
...
def create_simple_chain():
    ...
    chain.add_step(
        tool_name="my_calculator", input_template="{input}", output_key="result"
    )
